    ...
    <div className="rounded-xl h-fit">
      ...
      >
        <div className="flex  flex-grow items-center justify-between rounded-xl w-full sm:px-4 px-2 py-2 shadow-[0px_2px_10px_rgba(0,0,0,0.1)]">
          <div className="flex flex-col items-start text-color  ">
            <span className="sm:text-sm text-xs">
              {shortenYear(orderGroup.date)}
            </span>
            <span className="sm:text-xs text-xs text-color  ">
              {orderGroup.orderNumber === "Basket" ||
              orderGroup.orderNumber === "Корзина"
                ? orderGroup.orderNumber
                : `Номер: ${orderGroup.orderNumber}`}
            </span>
          </div>

          <span
            className={`text-xs sm:text-sm font-semibold ${
              orderGroup.status === t("status.paid")
                ? "text-[#EF4444]"
                : "text-[#C89A00]"
            }`}
          >
        ...
        <span className="text-[#49BA4A] text-[15px] text-color  sm:text-lg sm:min-w-[120px] rounded-xl  p-3 min-w-[85px]   text-center font-semibold shadow-[0px_2px_10px_rgba(0,0,0,0.1)]">
          {formattedPrice}
        </span>
      </button>
